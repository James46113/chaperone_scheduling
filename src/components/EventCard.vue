<template>
  <v-card class="my-3 mx-1" :elevation="small ? 0 : 3"
    :style="small ? 'border: 1px solid rgb(var(--v-theme-primary))' : ''">
    <v-row class="flex-nowrap">
      <v-col v-if="!small" cols="auto" max-width="200" class="mr-n4">
        <v-card class="pa-2 date" max-width="100px" min-width="60px" elevation="0" style="border-radius: 0;"
          height="100%">
      ...
      <v-col class="pt-5">
        <div class="d-flex flex-row flex-wrap justify-start" style="width: 100%;">
          <div class="flex-grow-1">
            <router-link :to="`/event/${props.event.id}`" class="routerlink">
              <v-card-title style="white-space: normal; overflow-wrap: anywhere; line-height: 1.1; ">
                {{ props.event.title }}
              </v-card-title>
              <v-card-subtitle v-if="!small" class="mt-n2 mb-2" style="white-space: normal; overflow-wrap: anywhere;">
                {{ props.event.location }}, {{ props.event.start.toLocaleTimeString([], {
                  hour: '2-digit', minute:
              ...
              <div v-else>
                <v-card-subtitle class="mt-n2 mb-1" style="white-space: normal; overflow-wrap: anywhere;">
                  {{ props.event.location }}
                </v-card-subtitle>
                <v-card-subtitle class="mt-n2 mb-2" style="white-space: normal; overflow-wrap: anywhere;">
              ...
              <v-card-text v-if="store.isAdmin" class="mt-n4">
            ...
            <availability-selector v-if="props.small" :event="props.event.id" small class="mt-n3 ml-1" @mousedown.stop
              @click.stop />
          </div>
          <v-expand-transition style="margin-top: -28px;">
            <div
              ...
              v-show="showTimeline" class="pa-2 pt-5">
              <v-fade-transition>
                <v-timeline v-show="showTimeline" direction="vertical" side="end" dot-color="primary" density="compact"
                  truncate-line="both">
                  ...
                  <template v-for="slot in props.event.slots" :key="slot.id">
                    <v-timeline-item v-if="slot.chaperone == props.chaperoneID" :size="16">
                      <div class="my-n2">
          ...
          <div
            v-if="showSlots && props.event.slots.filter(slot => slot.chaperone == props.chaperoneID).length > 0 && !isMobile"
            class="pa-2 mr-4">
            <v-timeline direction="horizontal" side="end" dot-color="primary" density="compact" truncate-line="both">
              <v-timeline-item :size="16">
                <div class="mt-n6 d-flex flex-column justify-center align-center" style="align-items: center;">
                  <b>
                    {{ props.event.start.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false })
                    }}
                  </b>
              ...
              <template v-for="slot in props.event.slots" :key="slot.id">
                <v-timeline-item v-if="slot.chaperone == props.chaperoneID" :size="16">
                  <div class="mt-n6 d-flex flex-column justify-center align-center" style="align-items: center;">
                    ...
                    <p class="mt-n1" style="text-align: center;">
                      {{ slot.title }}
                    </p>
                  </div>
                </v-timeline-item>
          ...
          <div v-if="props.getAvailability" class="ml-2" style="min-width: 200px; margin-left: auto">
            <availability-selector v-if="!props.small" :event="props.event.id" small
              :class="isMobile ? 'mt-n6' : 'mt-n3'" @mousedown.stop @click.stop />
          </div>
        </div>
      </v-col>
      <v-col v-if="isMobile && props.showSlots" cols="auto">
        <v-btn variant="text" color="primary" height="100%" @click="showTimeline = !showTimeline">
          <v-icon size="32">
