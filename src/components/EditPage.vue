<template>
  <app-header />
  <div v-if="!loadingData && !notFound" class="pa-6">
    <v-card class="pa-3" elevation="0">
      ...
      <v-row>
        <v-col>
          <v-text-field v-if="isTemplate" v-model="event.template_name" :width="isMobile ? '100%' : '65vw'"
            :rules="[required]" label="Template Name" variant="outlined" class="mb-1" :readonly="isDefaultTemplate" />

          <v-select v-if="!isTemplate && isNewEvent && isMobile" v-model="selectedTemplate" :items="store.templateNames"
            item-title="template_name" variant="outlined" placeholder="Load Template" width="100%"
            @update:model-value="loadTemplate" />

          <v-text-field v-model="event.title" :width="isMobile ? '100%' : '65vw'" :rules="[required]"
            label="Event Title" variant="outlined" class="mb-1" />

          <v-text-field v-model="event.location" :width="isMobile ? '100%' : '65vw'" :rules="[required]"
            label="Location" variant="outlined" class="mb-1" />
        </v-col>
        <v-col v-if="!isMobile">
          <div class="d-flex justify-end">
            <v-select v-if="!isTemplate && isNewEvent" v-model="selectedTemplate" :items="store.templateNames"
      ...
      <v-row v-if="!isMobile" class="mb-1">
        <v-col v-if="!isTemplate">
          <date-picker label="Date" :date="event.date?.toISOString().split('T')[0]" total-width="240px"
...
-->
      ...
      <v-textarea v-model="event.details" class="mt-6" label="Event Details (Optional)" variant="outlined" auto-grow
        rows="2" />

      <v-checkbox v-model="event.juniors_present" label="Juniors Present" color="primary" />
      ...
      <v-row class="mt-2 ml-2">
        ...
        <v-spacer />
        <v-col cols="auto">
          <v-select v-if="!isNewEvent && !isTemplate" v-model="event.lead_chaperone" class="mb-n2" variant="outlined"
            density="compact" label="Lead Chaperone" min-width="200" :items="on_duty_chaperones" item-title="name"
            item-value="id" clearable />
        </v-col>

        <v-col cols="auto">
          <v-btn v-if="!isMobile" color="primary" variant="outlined" class="mr-6" @click="newSlot">
            Add
            Chaperone
          </v-btn>
        </v-col>
      ...
      <v-card v-if="!isMobile && !isTemplate && !isNewEvent" elevation="0">
        ...
        <v-row class="mt-n3 mb-4">
          <v-col class="pl-7 pt-7">
            ...
            <v-card-text>
              <span v-if="event.availableChaperones?.length === 0" class="ml-2"><i>None available</i></span>
              <ul class="ml-7">
          ...
          <v-col class="pl-7 pt-7">
            ...
            <v-card-text>
              <span v-if="event.unavailableChaperones?.length === 0" class="ml-2"><i>None unavailable</i></span>
              <ul class="ml-7">
      ...
      <v-card-text v-if="event.slots?.length === 0 && isMobile" class="mt-4">
        <i>No chaperones assigned</i>
      </v-card-text>

      <v-divider class="my-4" />

      <v-sheet v-for="slot in event.slots" v-if="isMobile" color="primary" class="my-4" rounded style="padding: 1px;">
        <v-card class="pa-2">
          ...
          <v-textarea v-model="slot.details" label="Details (Optional)" variant="outlined" density="compact" auto-grow
            rows="2" class="mt-n2" />
          <v-select v-if="!isTemplate && !isNewEvent" v-model="slot.selectedChaperoneName" clearable
            :items="event.availability" item-title="chaperoneName" label="Chaperone" variant="outlined"
            density="compact" auto-select-first class="mt-n2">
            <template #item="{ props, item }" item-value="id">
              <v-list-item v-bind="props" :disabled="item.raw.available === false">
                <v-list-item-subtitle>
                  <v-chip variant="outlined" width="100px" height="100%" density="compact" size="small"
                    :color="item.raw.available ? 'green' : item.raw.available === false ? 'error' : 'orange'">
          ...
          <v-row>
            ...
            <v-spacer />
            <time-picker :hours="slot.startHours" :minutes="slot.startMinutes" class="mr-3"
              @update:hours="slot.startHours = $event" @update:minutes="slot.startMinutes = $event" />
          </v-row>

          <v-row class="mt-n5">
            <span class="mt-4 ml-7">End</span>
            <v-spacer />
            <time-picker :hours="slot.endHours" :minutes="slot.endMinutes" class="mr-3"
              @update:hours="slot.endHours = $event" @update:minutes="slot.endMinutes = $event" />
          </v-row>

          <v-btn variant="outlined" width="100%" color="primary" @click="removeSlot(slot)">
            Remove
            Chaperone
          </v-btn>
        </v-card>
      ...
      <div v-if="isMobile">
        <v-btn color="primary" variant="outlined" width="100vw" class="mt-6 mr-6" @click="newSlot">
          Add
          Chaperone
        </v-btn>
  ...
  <div v-else-if="notFound" class="d-flex justify-center mt-10">
    <v-card class="ma-4 pa-4" :width="isMobile ? '100vw' : '40vw'">
      <v-img src="/Steel-City-Choristers.png" width="15vw" />
      ...
      <div class="d-flex justify-center">
        <v-btn v-if="!isTemplate" variant="flat" width="20%" color="primary" @click="proxy.$router.push('/')">
          Events
        </v-btn>
        <v-btn v-else variant="flat" width="20%" color="primary" @click="proxy.$router.push('/templates')">
          Templates
        </v-btn>
      </div>
    </v-card>
  ...
  <v-dialog v-model="showConfirmDeleteDialog" :width="isMobile ? '100vw' : '30vw'">
    <v-card @keyup.enter="startDeleteEvent">
      ...
      <v-card-actions>
        <v-btn color="primary" variant="text" @click="showConfirmDeleteDialog = false">
          Cancel
        </v-btn>
        <v-btn :loading="deleting" color="primary" variant="flat" @click="startDeleteEvent">
          Delete
        </v-btn>
      </v-card-actions>
    </v-card>
